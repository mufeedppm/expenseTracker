...
window.addEventListener('DOMContentLoaded',async ()=>{
    try{
        let page=1
        let getReq =await axios.get(`http://50.19.247.142:3000/expense?page=${page}&items=${itemsPerPage.value}`, {headers: {'Authorization': token}})
...
function pagination(obj){
    ...
    if(obj.hasPrevPage){
        ...
        btn.innerHTML= `<h3 class=pg>${obj.currentPage}</h3>`
        expenseList.innerHTML='<h4>Expenses</h4>';
...
async function addExpense(e){
    try{
        ...
        // else{
            ...
            let postReq= await axios.post("http://50.19.247.142:3000/expense/addExpense",obj, {headers: {'Authorization': token}})
...
async function displayExpense(obj){
    try{
        ...
        editBtn.className='btn-e btn btn-right'
        ...
        delBtn.className=' btn-e btn  btn-right btn-danger'
...
async function editExpense(obj){
    try{
        if(confirm("Please confirm to edit")){
            ...
            await axios.delete(`http://50.19.247.142:3000/expense/deleteExpense/${obj.id}`)
...
async function deleteExpense(key){
    try{
        if(confirm("Press OK to confirm delete")){
            ...
            const delReq = await axios.delete(`http://50.19.247.142:3000/expense/deleteExpense/${key}`,{headers: {'Authorization': token}})
...
leaderBtn.classList='leaderBoard btn btn-right'
...
leaderBtn.onclick =async function leaderBoard() {
    try{
        ...
        const resp =await axios.get('http://50.19.247.142:3000/premium/leaderboard', {headers: {'Authorization': token}})
...
document.getElementById('rzp-btn1').onclick = async function(e) {
 try{
    ...
    let response = await axios.get("http://50.19.247.142:3000/purchase/premium", {headers: {'Authorization': token}})
    ...
    "handler": async function(response) {
        await axios.post("http://50.19.247.142:3000/purchase/update-transaction",{
...
rzp1.on('payment.failed',async function(){
    try{
        ...
        const resp=await axios.post("http://50.19.247.142:3000/purchase/update-transaction",{
                "orderId":response.data.order.id,
                "payment_id":null
            },{headers:{"Authorization":token} })
...
reportBtn.id='report'
reportBtn.classList='report btn btn-right'
...
reportBtn.onclick = async function expenseReport(){
    try{
        ...
        const response = await axios.get('http://50.19.247.142:3000/expense/download', { headers: {"Authorization" : token} })
